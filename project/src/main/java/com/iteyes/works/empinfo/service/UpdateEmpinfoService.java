...
public class UpdateEmpinfoService {
	...
	public String updatePasswd(Map<String, String> passwdParam) throws Exception{
		
		String result = "";
		Map<String, String> param = new HashMap<>();
		String origin_passwd = AES256Security.encrypt(passwdParam.get("origin_passwd"));
		param.put("emp_no", "2");
		param.put("passwd", origin_passwd);
		
		int first_result = updateMapper.selectPasswd(param);
		if(first_result == 1) {
			
			String new_passwd = AES256Security.encrypt(passwdParam.get("new_passwd"));
			param.put("passwd", new_passwd);
			int second_result = updateMapper.updatePasswd(param);
			if(second_result == 1) {
				result = "1";
			}else {
				result = "3";
	...
	public void updateEmpBasicinfo() throws Exception{
		...
		String rsdn_reg_no = param.get("data[rsdn_reg_no_1]")+param.get("data[rsdn_reg_no_2]");
